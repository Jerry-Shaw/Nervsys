...
class Linux
{
    ...
    public function getHwHash(): string
    {
        $cmd =
            ...
            'lscpu 2>/dev/null | awk -F: \'{gsub(/^[ \t]+/,"",$1); if($1=="Architecture" || $1=="CPU(s)" || $1=="Thread(s) per core" || $1=="Core(s) per socket" || $1=="Socket(s)" || $1=="Vendor ID" || $1=="Model name" || $1=="Stepping") {gsub(/^[ \t]+/,"",$2); printf "%s ",$2}} END {print ""}\'; ' .
