...
define('JSON_FORMAT', JSON_INVALID_UTF8_IGNORE | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_LINE_TERMINATORS);
        ...
        if (!empty($cgi_cmd)) {
            try {
                ...
                if ($pass_hook) {
                    ...
                    $this->profiler->start($profiler_name);
                    $this->IOData->src_output = $this->caller->runApiFn($cgi_cmd, $this->IOData->src_input, !$this->app->is_cli);
                    $this->profiler->end($profiler_name);
                }
            } catch (\Throwable $throwable) {
                $this->error->exceptionHandler($throwable);
