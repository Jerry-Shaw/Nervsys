...
在CGI模式下，也就是大家所知的HTTP请求，因为一些安全因素考虑，"c" 永远都是默认重定向到api路径下，但是，CLI模式下允许直接调用root路径下命令，只需要在"c"前面加上"/"，并使用完整的类命名空间路径即可，这个模式，相当于在入口文件中使用"libCoreApi::new()->openRootExec(true)"，设置路由类中的"open_root_exec"为true，关闭api接口重定向。  
