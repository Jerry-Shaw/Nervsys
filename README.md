...
****Format for test_1.php:** test_1.php 模板格式
        
    //The right namespace follows the path structure             根据脚本路径定义命名空间
    namespace pr_1\ctr;
        
    //Any other extensions and namespaces can be used here      use 其他的扩展和命名空间
    use ext\http;
        
    //Class name should be exactly the same as the file name        类名保持和文件名一致  
    class test_1
    {
        /**
        * Important!!!
...
CLI命令行用法：
    ...
    c/cmd: command
    d/data: CGI data content            CGI数据内容
    p/pipe: CLI pipe content            CLI管道内容 
    r/record: record type (result (default) / error / data / cmd, multiple options) 记录类型
    t/timeout: timeout for return (in microseconds, default value is 5000ms when r/return is set)   返回的超时设置（以ms为单位，如果r/return有设置，默认5000ms）
    l/log: log option                           日志选项
