...
添加 FTS5 全文搜索的 `MATCH` 条件。每个条件是一个数组：`[列名, 关键词]`，可选在数组开头插入 `'AND'` 或 `'OR'` 作为连接符（默认
`AND`）。多个条件会按顺序用各自的连接符组合成复合 `MATCH` 子句，并自动用括号包裹以避免与外层 `WHERE` 冲突。

- **参数:** 可变数量的条件数组，每个数组格式为 `[连接符?, 列名, 关键词]`。
- **返回:** 当前实例以支持链式调用。
