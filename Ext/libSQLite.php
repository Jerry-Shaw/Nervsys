...
class libSQLite extends Factory
{
    public \PDO          $pdo;
    ...
    /**
     * Auto reconnect with limited times (SQLite doesn't support reconnect, kept for interface compatibility)
     *
            ...
            if (is_string($val) && in_array(strtoupper($val), ['ASC', 'DESC'], true)) {
                ...
                if (is_string($val)) {
                    ...
                    if (!in_array($last_val, ['ASC', 'DESC'], true)) {
                        $order[] = $this->buildFieldOrder($col, implode(',', $val));
                    } else {
                        array_pop($val);
                        $order[] = $this->buildFieldOrder($col, implode(',', $val)) . ' ' . $last_val;
                    }
                }
            }
        }
    ...
    /**
     * Set lock mode (SQLite doesn't support FOR UPDATE/SHARE, kept for interface compatibility)
     *
     * @param string ...$modes
     *
     * @return $this
    ...
    {
        // SQLite doesn't support FOR UPDATE/SHARE lock modes
    ...
    /**
     * Explain SQL - SQLite uses EXPLAIN QUERY PLAN
     *
    ...
    {
        return $this->query('EXPLAIN QUERY PLAN ' . $readable_sql)->fetchAll(\PDO::FETCH_ASSOC);
    }

    /**
     * Reconnect (SQLite doesn't support reconnect, kept for interface compatibility)
     *
     * @param int $retry_times
     *
     * @return bool
    ...
    {
        // SQLite doesn't have connection errors like MySQL (2006, 2013)
        // Always return false for SQLite
        unset($retry_times);
        return false;
    }
    ...
    /**
     * Build SQL for REPLACE INTO (SQLite uses INSERT OR REPLACE)
     *
     * @return string
     */
    protected function buildReplace(): string
    {
        $sql = 'INSERT OR REPLACE INTO ' . $this->getTableName();
        $sql .= ' (' . implode(',', $this->runtime_data['cols']) . ')';
        $sql .= ' VALUES (' . implode(',', array_pad([], count($this->runtime_data['bind']), '?')) . ')';
    ...
    /**
     * Build SQL for REPLACE INTO ... SET (SQLite uses INSERT OR REPLACE)
     *
     * @return string
     */
    protected function buildReplaceSet(): string
    {
        return 'INSERT OR REPLACE INTO ' . $this->getTableName() . ' ' . $this->getSqlSet();
    }
    ...
    protected function parseCond(array $where, string $option): array
    {
        ...
        foreach ($where as $value) {
            ...
            if (2 <= count($value)) {
                ...
                if (!in_array($item, ['+', '-', '*', '/', '%', '=', '<=>', '<', '>', '<=', '>=', '<>', '!=', '|', '&', '^', '~', '<<', '>>', 'MOD', 'LIKE', 'NOT LIKE', 'IN', 'NOT IN', 'BETWEEN', 'GLOB'], true)) {
